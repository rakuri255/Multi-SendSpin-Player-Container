...
public class MockRelayBoardFactory : IRelayBoardFactory
{
    ...
    private static int GetMockChannelCount(string boardId, RelayBoardType boardType)
    {
        ...
        return boardId switch
        {
            ...
            _ when boardId.StartsWith("MODBUS:", StringComparison.OrdinalIgnoreCase) => 16,
            _ when boardId.StartsWith("GPIO:", StringComparison.OrdinalIgnoreCase) => 8, // GPIO boards default to 8
            _ => boardType switch
            {
                RelayBoardType.UsbHid => 4,
                RelayBoardType.Modbus => 16,
                RelayBoardType.RaspberryPiGpio => 8,
                _ => 8
            }
        };
    }
